...
const filePathExists = (path) => fs.existsSync(path)
...
const storagePath = (() => {
  ...
  if (!pathContainsBeginningSlash) return nodePath.normalize('/' + systemAgnosticPath)
...
app.get('/downloadZip', (req, res) => {
  ...
  if (!filesAreAvailableToDownload) return res.status(400).send()
...
app.post('/uploadAuth', upload.array('uploaded-files'), (req, res) => {
  ...
  if (!filesAreAvailableToUpload) return res.status(500).redirect('/upload')
  return res.status(200).send()
})

app.post('/download', (req, res) => {
  const listOfFilesExist = isArrayValid(req.body.fileNames)
  if (!listOfFilesExist) return res.status(400).send('There is no list of files to download.')
  const files = []
...
app.delete('/deleteMultiple', (req, res) => {
  ...
  if (!listOfFilesExist) return res.status(400).send('There are no files to delete.')
